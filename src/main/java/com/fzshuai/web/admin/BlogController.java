...
/**
 * @author 软件二班傅同学
 * @date 2021-01-22 22:49
 */
@Controller
@RequestMapping("admin")
public class BlogController<asi> {
    ...
    @GetMapping("/blogs")
    public String blogs(@PageableDefault(size = 2, sort = {"updateTime"}, direction = Sort.Direction.ASC) Pageable pageable, BlogQuery blog, Model model) {
    ...
    public String search(@PageableDefault(size = 2, sort = {"updateTime"}, direction = Sort.Direction.ASC) Pageable pageable, BlogQuery blog, Model model) {
        model.addAttribute("page", blogService.listBlog(pageable, blog));
